...
<div class="main">
    ...
    <div class="main-right" id="buy_person" style="height: 900px">
        <a href="code_project.action"><h3 style="display: inline-block"><span class="overview" id="public">myProject_public</span>
        </h3></a>
        <a href="code_project.action?status=private"><h3 style="display: inline-block"><span class="overview2" id="private">myProject_private</span>
        </h3></a>
        <a href="code_Creatrep.action" title="Creat a Project" target="_blank"><img
        ...
        <div style="overflow: hidden" class="CodeModule">
            ...
            <%--隐藏一个参数--%>
            <input type="hidden" id="status" value="<s:property value="#parameters.status"/>">
        </div>
    </div>
</div>
<div class="footer">
...
<script>
    $(function () {
        ...
        if (status != "") {
            $("#private").attr("class","overview");
            $("#private").attr("style","margin-left: 35px");
            $("#public").attr("class","");
            $("#public").attr("style","padding-bottom: 8px");
        }
    })
    //分页
    $(".page_div").paging({
            pageNo: ${repertories.currentPage},
            totalPage: ${repertories.totalPage},
            totalSize: ${repertories.totalCount},
            callback: function (num) {
                var status = $("#status").val();
                if (status != "") {
                    window.location.href = "code_project?currentPage=" + num + "&status=" + status;
                } else {
                    window.location.href = "code_project?currentPage=" + num;
                }
            }
        }
    )
