...
<div class="main" style="height: 850px">
    <div class="main-left">
        ...
        <div class="photo">
            <c:choose>
                <c:when test="${empty userById.head_image}">
                    <img src="" id="user_picture">
                    <p id="nophoto">No Photo</p>
                </c:when>
                <c:otherwise>
                    <img src="upload/${userById.head_image}" id="user_picture">
                </c:otherwise>
            </c:choose>
        </div>
        <div class="name">
            ...
            <p>Age:&nbsp;&nbsp;${userById.age}</p>
        </div>

        <div class="bio" style="top: 330px">
            <p>self_info:</p>
            <div class="intro">
                <c:choose>
                    <c:when test="${empty userById.self_info}">
                        <p style="margin-top: 5px">
                            You haven't written your profile yet...
                        </p>
                    </c:when>
                    <c:otherwise>
                        <p style="margin-top: 8px;margin-left: 8px">${userById.self_info}</p>
                    </c:otherwise>
                </c:choose>
            </div>
        </div>
    ...
    <div class="main-right" id="buy_person" style="height: 700px">
        <a href=""><h3 style="display: inline-block"><span class="overview">${userById.username}&nbsp;&nbsp;CreatModule</span></h3></a>
        <div style="width: 600px;height: 1px;background-color: #BBBBBB;margin-top: 18px;"></div>
        <div style="overflow: hidden" class="CodeModule">
            <!--个人代码模块-->
            <c:forEach items="${oth_pageBean.pageList}" var="code">
                <div class="repositories-1" style="height:95px">
                    <a href="code_CodeDetail.action?id=${code.id}" target="_blank">${code.code_name}</a>
                    ...
                    <div class="management">
                        <a href="code_CodeDetail.action?id=${code.id}" target="_blank">View</a>
                    </div>
                </div>
            </c:forEach>
            <div class="pagepage">
...
<script>
    //分页
    $(".page_div").paging({
            pageNo: ${oth_pageBean.currentPage},
            totalPage: ${oth_pageBean.totalPage},
            totalSize: ${oth_pageBean.totalCount},
            callback: function (num) {
                window.location.href = "code_otherModule.action?uid=${codeuser.id}&currentPage=" + num;
            }
        }
    )
    $(function () {
